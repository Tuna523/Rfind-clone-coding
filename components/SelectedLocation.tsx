...
const SelectedLocation:React.FC<{
    ...
    value.map((text,idx)=>{
        return(
            <div onClick={(e)=>{console.log()}} className="font-normal Chip_component Chip_medium Chip_dark" tabIndex={-1} style={{boxSizing: 'border-box'}} key={idx}>
                <div tabIndex={-1} style={{boxSizing: 'border-box', height: '100%', minHeight: '100%', width: '100%', placeContent: 'flex-start', alignItems: 'center', display: 'flex', flexFlow: 'row nowrap', gap: '10px'}}>
                    ...
                    <div onClick={(e)=> {e.preventDefault(); removeValueHandler(idx)}} className="Chip_delete" role={'button'} tabIndex={0} style={{boxSizing: 'border-box', cursor: 'pointer'}}>
